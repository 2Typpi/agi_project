...
class MinesweeperEnv(object):
    ...
    """
    Clicks the tile on the board and uncovers all allowd adjacent tiles

    Args: 
        integer: index of the field to click when board is flat and enumerated
    Returns:
        state of the board as a np.array in shape (W x H)
        reward as a float
        done as a Boolean True if won or lost 
    """
